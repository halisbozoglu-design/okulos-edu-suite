...
Real same-runner workflow `33162816626`, artifact `9683469554`, ran Timefold 2.5.0, FET 6.8.5 and official UniTime CPSolver pinned at `3abbcaaf26d739d25e45c8e191b7ef94bc15cc26` over six profiles × 30 seeds each. Timefold and UniTime each completed 180 real runs as `RUN_COMPARABLE`: both in-engine searches use the canonical HARD → unplaced → MEDIUM → SOFT ordering, and independent canonical post-score gates matched all four levels on every row. Both achieved feasible_rate=1, HARD=0 and unplaced=0; UniTime additionally passed isolated fresh-JVM deterministic replay. Budget gates passed. Frozen evidence: `benchmarks/world/external-same-runner-20260828-v2.json`. Earlier Timefold-only comparable evidence remains immutably preserved in `benchmarks/world/external-same-runner-20260828.json`.

FET remains `RUN_COMMON_HARD`, not `RUN_COMPARABLE`: its actual search objective has not been proven identical to the canonical lexicographic objective, so its post-scored MEDIUM/SOFT values cannot be used as apples-to-apples objective or runtime superiority evidence. aSc remains `NOT_RUN`; no documented repeatable headless/CLI generation entrypoint has been verified. A conservative aSc 2012 XML input adapter remains `INPUT_PARTIAL`. Synthetic MTAL/MESEM are structural profiles, not anonymized real-school data.

Therefore Section 13 remains closed as the reproducible benchmark package; Section 15 alone owns parity/superiority closure. Timefold and UniTime objective parity are proven for this corpus, but full cross-solver superiority remains prohibited while FET is non-comparable and aSc is unrun.
...
- Timefold 2.5.0 exact benchmark objective is independently proven `RUN_COMPARABLE`.
- UniTime CPSolver pinned at `3abbcaaf26d739d25e45c8e191b7ef94bc15cc26` is independently proven `RUN_COMPARABLE`: same-runner workflow `33162816626`, artifact `9683469554`, 180/180 real runs, objective parity=true, feasible_rate=1, HARD=0, unplaced=0, fresh-process deterministic replay=true and budget gates pass. Frozen evidence is `benchmarks/world/external-same-runner-20260828-v2.json`.
...
2. **FET identical external objective:** FET remains `RUN_COMMON_HARD`. It may be upgraded only by genuine in-engine optimization proven equivalent to the same HARD → unplaced → MEDIUM → SOFT contract plus independent canonical parity—not by post-score relabeling.
3. **Final statistics:** after the remaining comparable/executable evidence, require 0 HARD, equal/higher feasible rate, deterministic/provenanced runs, and Pareto superiority or statistical equivalence. Product advantage for MEB/MTAL/MESEM must be independently demonstrated rather than inferred from benchmark runtime.
