...
Real same-runner workflow `33128382948`, artifact `9669826383`, executed Timefold 2.5.0, FET 6.8.5 and official UniTime CPSolver pinned at `3abbcaaf26d739d25e45c8e191b7ef94bc15cc26` over six profiles × 30 seeds each. Timefold completed all 180 real runs as `RUN_COMPARABLE`: its native bendable objective genuinely optimizes canonical HARD → unplaced → MEDIUM → SOFT, and an independent canonical post-score gate matched all four levels on every row. Timefold feasible_rate=1, HARD=0, unplaced=0, deterministic replay=true and budget_pass=true. Frozen evidence: `benchmarks/world/external-same-runner-20260828.json`. Direct Timefold workflow `33128231487` also completed successfully as corroborating executable evidence.

FET and UniTime remain `RUN_COMMON_HARD`, not `RUN_COMPARABLE`: their actual search objectives have not yet been proven identical to the canonical lexicographic objective. Their post-scored MEDIUM/SOFT values therefore cannot be used as apples-to-apples objective or runtime superiority evidence. aSc remains `NOT_RUN`; no documented repeatable headless/CLI generation entrypoint has been verified. A conservative aSc 2012 XML input adapter remains `INPUT_PARTIAL`. Synthetic MTAL/MESEM are structural profiles, not anonymized real-school data.

Therefore Section 13 remains closed as the reproducible benchmark package; Section 15 alone owns parity/superiority closure. Timefold objective parity is proven for this corpus, but cross-solver superiority remains prohibited while FET/UniTime are non-comparable and aSc is unrun.
...
- Timefold 2.5.0 exact benchmark objective is now independently proven: workflow `33128382948`, artifact `9669826383`, 180/180 real runs `RUN_COMPARABLE`, objective parity=true, feasible_rate=1, HARD=0, unplaced=0, deterministic replay and budget pass. Frozen evidence is `benchmarks/world/external-same-runner-20260828.json`.
- Latest source chain before truth-sync passed CI `33128382918`: unit/regression, CP-SAT, 30-seed world benchmark, parity truth gate, migration/replay/tenant/auth/route guards, production build, route tree, TypeScript and forward migration policy all SUCCESS.
...
2. **Remaining identical external objectives:** Timefold is now `RUN_COMPARABLE`; FET and UniTime remain `RUN_COMMON_HARD`. Either must be upgraded only by genuine in-engine optimization of the same HARD → unplaced → MEDIUM → SOFT contract plus independent canonical parity—not by post-score relabeling.
3. **Final statistics:** after remaining comparable execution, require 0 HARD, equal/higher feasible rate, deterministic/provenanced runs, and Pareto superiority or statistical equivalence. Product advantage for MEB/MTAL/MESEM must be independently demonstrated rather than inferred from benchmark runtime.
