...
const dir=join(process.cwd(),"supabase","migrations"),files=readdirSync(dir).filter(f=>f.endsWith(".sql")).sort();
...
"function:validate_schedule_scenario_v2":"20260826231000_schedule_generic_constraint_parity.sql",
...
"function:create_schedule_restore_point_permission_core_v2":"20260902000600_schedule_restore_tenant_chain_v2.sql",
"function:create_schedule_restore_point_v2":"20260902000600_schedule_restore_tenant_chain_v2.sql",
"function:restore_schedule_restore_point_v2":"20260902000600_schedule_restore_tenant_chain_v2.sql",
...
const lastDefinition=new Map();for(const file of files){const sql=readFileSync(join(dir,file),"utf8");for(const key of Object.keys(protectedDefinitions)){const[kind,name]=key.split(":");const pattern=kind==="view"?new RegExp(`create\\s+or\\s+replace\\s+view\\s+public\\.${name}\\b`,"i"):new RegExp(`create\\s+or\\s+replace\\s+function\\s+public\\.${name}\\s*\\(`,"i");if(pattern.test(sql))lastDefinition.set(key,file)}}
const errors=[];for(const[key,expected]of Object.entries(protectedDefinitions)){const actual=lastDefinition.get(key);if(!actual)errors.push(`${key}: tanım bulunamadı`);else if(actual!==expected)errors.push(`${key}: son tanım ${actual}, beklenen ${expected}`)}if(errors.length){console.error("Timetable authority check FAILED:\n"+errors.map(e=>`- ${e}`).join("\n"));process.exit(1)}console.log(`Timetable authority check OK (${Object.keys(protectedDefinitions).length} protected definitions).`);
