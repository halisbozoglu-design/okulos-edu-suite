...
public final class TimefoldWorldBenchmark {
  ...
  static ObjectNode run(JsonNode c,boolean replay){long mem0=used(),t0=System.nanoTime();Timetable r;try{r=solve(c);}catch(Exception e){var z=M.createObjectNode();z.put("solver_id","timefold");z.put("input_hash",c.get("input_hash").asText());z.put("profile_id",c.get("profile_id").asText());z.put("seed",c.get("seed").asLong());z.put("status","ERROR");z.put("feasible",false);z.set("diagnostics",M.createObjectNode().put("error",e.toString()));return z;}long ms=Math.round((System.nanoTime()-t0)/1_000_000.0);int unplaced=(int)r.lessons.stream().filter(l->l.slot==null).count(),hard=-r.score.hardScore();boolean feasible=unplaced==0&&hard==0;var z=M.createObjectNode();z.put("solver_id","timefold");z.put("solver_version","2.5.0");z.put("mapping","HARD_COMMON_CORE");z.put("comparable_objective",false);z.put("input_hash",c.get("input_hash").asText());z.put("profile_id",c.get("profile_id").asText());z.put("seed",c.get("seed").asLong());z.put("status",ms>c.get("wall_clock_budget_ms").asLong()+1500?"TIMEOUT":"COMPLETED");z.put("feasible",feasible);z.put("hard",hard);z.put("unplaced",unplaced);z.put("runtime_ms",ms);z.put("time_to_first_feasible_ms",feasible?ms:-1);z.put("time_to_best_ms",ms);z.put("peak_memory_mb",Math.max(0,(used()-mem0)/1048576));var rows=M.createArrayNode();for(var l:r.lessons)if(l.slot!=null){var q=M.createObjectNode();q.put("assignment_id",l.id);q.put("weekday",l.slot.day);q.put("period",l.slot.period);rows.add(q);}z.set("rows",rows);if(replay){var r2=solve(c);z.put("deterministic_replay",sig(r).equals(sig(r2))&&r.score.equals(r2.score));}return z;}
  static long used(){var x=Runtime.getRuntime();return x.totalMemory()-x.freeMemory();}
  public static void main(String[] args)throws Exception{if(args.length<2)throw new IllegalArgumentException("usage: <cases.ndjson> <out.ndjson>");try(var in=Files.lines(Path.of(args[0]));var out=Files.newBufferedWriter(Path.of(args[1]))){var first=new HashSet<String>();for(var it=in.iterator();it.hasNext();){var c=M.readTree(it.next());boolean replay=first.add(c.get("profile_id").asText());out.write(M.writeValueAsString(run(c,replay)));out.newLine();out.flush();}}}
}
