...
function LoginForm(){
  ...
  if(mode==="password")return <form className="space-y-4" onSubmit={passwordLogin}><div className="space-y-2"><Label>E-posta veya T.C. Kimlik No</Label><Input value={identifier} onChange={e=>setIdentifier(e.target.value)} autoComplete="username" placeholder="ornek@posta.com veya 11 haneli T.C."/></div><div className="space-y-2"><Label>Şifre</Label><Input type="password" value={password} onChange={e=>setPassword(e.target.value)} autoComplete="current-password"/></div>{error?<ErrorNotice message={error}/>:null}{success?<SuccessNotice message={success}/>:null}<Button type="submit" size="lg" className="w-full gap-2" disabled={busy}>Giriş Yap <ArrowRight className="size-4"/></Button><div className="flex items-center justify-between text-xs"><button type="button" className="text-primary hover:underline" onClick={()=>resetMode("forgot")}>Şifremi unuttum</button><button type="button" className="text-primary hover:underline" onClick={()=>resetMode("otp")}>Kod ile gir</button></div></form>;
