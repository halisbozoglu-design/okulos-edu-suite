        ...
        <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
          <Tabs defaultValue="signup">
            <TabsList className="w-full">
              ...
              <TabsTrigger value="signup" className="flex-1">Üye Ol</TabsTrigger>
            </TabsList>
          ...
          <p className="mt-5 flex items-start gap-2 text-xs text-muted-foreground">
            ...
            Kimlik ve profil verileri yetki kurallarıyla korunur.
          </p>
        </div>
      </div>
    </div>
...
function LoginForm() {
  ...
  async function sendLoginCode() {
    ...
    setBusy(false);
    if (authError) {
      setError("Giriş kodu gönderilemedi. Bilgilerinizi kontrol ediniz.");
      return;
    }
    setCodeSent(true);
  }
    ...
    <form className="space-y-4" onSubmit={verifyLogin}>
      <div className="space-y-2">
        ...
        <div className="flex gap-2">
          <Input id="login-email" type="email" autoComplete="email" value={email} disabled={codeSent} onChange={(e) => setEmail(e.target.value)} />
          <Button type="button" onClick={sendLoginCode} disabled={busy}>{codeSent ? "Tekrar Gönder" : "Kod Gönder"}</Button>
        </div>
      </div>
      {codeSent ? (
        <div className="space-y-2">
          <Label>Doğrulama Kodu</Label>
          <InputOTP maxLength={6} value={otp} onChange={setOtp}><InputOTPGroup>{[0,1,2,3,4,5].map((i) => <InputOTPSlot key={i} index={i} />)}</InputOTPGroup></InputOTP>
        </div>
      ) : null}
...
function SignUpFlow() {
  ...
  async function submitTckn(e: React.FormEvent) {
    e.preventDefault();
    if (!/^\d{11}$/.test(tckn)) return setError("T.C. Kimlik No tam olarak 11 haneli olmalıdır.");
    setBusy(true);
    setError(null);
    const { data, error: fnError } = await supabase.functions.invoke("validate-teacher", { body: { tckn } });
    setBusy(false);
  ...
  async function finalize(e: React.FormEvent) {
    ...
    if (authError || !data.user) {
      setBusy(false);
      return setError("Doğrulama kodu geçersiz veya süresi dolmuş.");
    }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        <span className="text-xs font-medium text-muted-foreground">{step === 1 ? "Adım 1 / 2 · Kimlik Doğrulama" : "Adım 2 / 2 · E-posta Onayı"}</span>
        <div className="ml-auto flex gap-1"><span className="h-1.5 w-6 rounded-full bg-primary" /><span className={step === 2 ? "h-1.5 w-6 rounded-full bg-primary" : "h-1.5 w-6 rounded-full bg-muted"} /></div>
      </div>
      {step === 1 ? (
        <form className="space-y-4" onSubmit={submitTckn}>
          <div className="space-y-2">
            <Label htmlFor="tckn">T.C. Kimlik No</Label>
            <Input id="tckn" inputMode="numeric" autoComplete="off" maxLength={11} value={tckn} onChange={(e) => setTckn(e.target.value.replace(/\D/g, "").slice(0, 11))} />
            <p className="text-xs text-muted-foreground">{tckn.length}/11 hane</p>
          </div>
          {error ? <ErrorNotice message={error} /> : null}
          <Button type="submit" size="lg" className="w-full gap-2" disabled={busy}>Devam Et <ArrowRight className="size-4" /></Button>
        </form>
      ) : (
        <form className="space-y-4" onSubmit={finalize}>
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Input id="signup-email" type="email" value={email} disabled={codeSent} onChange={(e) => setEmail(e.target.value)} />
              <Button type="button" onClick={sendCode} disabled={busy}>{codeSent ? "Tekrar Gönder" : "Kod Gönder"}</Button>
            </div>
          </div>
          {codeSent ? (
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-xs text-muted-foreground"><MailCheck className="size-3.5 text-primary" />6 haneli kod e-posta adresinize gönderildi.</div>
              <Label>Doğrulama Kodu</Label>
              <InputOTP maxLength={6} value={otp} onChange={setOtp}><InputOTPGroup>{[0,1,2,3,4,5].map((i) => <InputOTPSlot key={i} index={i} />)}</InputOTPGroup></InputOTP>
            </div>
          ) : null}
          {error ? <ErrorNotice message={error} /> : null}
          <div className="flex gap-2">
            <Button type="button" variant="ghost" onClick={() => { setStep(1); setError(null); }}>Geri</Button>
            <Button type="submit" size="lg" className="flex-1" disabled={!codeSent || busy}>Kaydı Tamamla</Button>
          </div>
        </form>
      )}
    </div>
