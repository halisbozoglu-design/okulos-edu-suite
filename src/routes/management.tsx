...
type ManagementItem={to:string;title:string;desc:string;icon:LucideIcon;permissions:readonly string[];superOnly?:boolean;principalOnly?:boolean;principalOrSuper?:boolean;group:Group};
...
const items:readonly ManagementItem[]=[
 ...
 {group:"data",to:'/personnel-import',title:'Personel Özet Bilgi İçe Aktarma',desc:'MEBBİS PDF/Excel tüm personel alanları — yalnız kurum müdürü',icon:ShieldCheck,permissions:[],principalOnly:true},
 ...
 {group:"system",to:'/personnel-field-settings',title:'Personel Alan Kullanımı',desc:'Süper Admin varsayılanı; kurum müdürü kendi kurumunun etkin alanlarını belirler',icon:Crown,permissions:[],principalOrSuper:true},
...
function ManagementHub(){
 const [codes,setCodes]=useState<Set<string>|null>(null),[isSuper,setIsSuper]=useState(false),[isPrincipal,setIsPrincipal]=useState(false),[error,setError]=useState<string|null>(null);
 useEffect(()=>{void(async()=>{const [p,s,principal]=await Promise.all([supabase.rpc('get_my_permissions'),supabase.rpc('is_super_admin'),supabase.rpc('can_manage_personnel_private_data')]);if(p.error){setError('Görev/yetki bilgileri okunamadı.');setCodes(new Set());return;}setCodes(new Set((p.data??[]).map((x:{code:string})=>x.code)));setIsSuper(Boolean(s.data));setIsPrincipal(Boolean(principal.data));})()},[]);
