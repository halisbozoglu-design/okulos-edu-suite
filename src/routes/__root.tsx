...
const protectedRoutes: RouteRule[] = [
  ...
  { prefix: "/academic-years", any: ["settings.manage", "schedule.view", "schedule.rules", "schedule.generate"] },
...
function SystemAccessBoundary({children}:{children:ReactNode}){
  ...
  useEffect(()=>{let alive=true;if(publicPaths.includes(pathname)){setState({allowed:true});return()=>{alive=false}};setState(null);void(async()=>{const {data:userData}=await supabase.auth.getUser();if(!alive)return;if(!userData.user){setState({allowed:false,reason:"not_authenticated",message:"Bu sayfayı açmak için giriş yapmalısınız."});return;}const {data,error}=await supabase.rpc("get_system_access_state",{p_path:pathname});if(!alive)return;if(error){setState({allowed:false,reason:"access_check_failed",message:"Sistem/tenant erişim kontrolü tamamlanamadı. Güvenlik nedeniyle erişim açılmadı; lütfen tekrar deneyin."});return;}setState((data??{allowed:false,reason:"access_check_failed"}) as AccessState)})();return()=>{alive=false}},[pathname]);
  ...
  const pending=state.reason==="tenant_pending",rejected=state.reason==="tenant_rejected",maintenance=state.reason==="system_maintenance"||state.reason==="feature_maintenance",notAuthenticated=state.reason==="not_authenticated",tenantRequired=state.reason==="tenant_required"||state.reason==="tenant_inactive";
  return <div className="flex min-h-screen items-center justify-center bg-background px-4"><div className="w-full max-w-xl rounded-2xl border bg-card p-7 text-center shadow-sm"><div className="mx-auto grid size-12 place-items-center rounded-2xl bg-amber-100 text-amber-800">{maintenance?"🛠️":pending?"⏳":rejected?"✕":tenantRequired?"🏫":notAuthenticated?"🔐":"!"}</div><h1 className="mt-4 text-xl font-semibold">{maintenance?(state.feature_label?`${state.feature_label} bakımda / kapalı`:`Sistem bakımı yapılmaktadır`):pending?"Kurum onayı bekleniyor":rejected?"Kurum kaydı onaylanmadı":tenantRequired?"Aktif kurum bağlantısı gerekli":notAuthenticated?"Giriş gerekli":"Erişim güvenlik nedeniyle kapalı"}</h1><p className="mt-2 text-sm leading-relaxed text-muted-foreground">{state.message??(pending?"Okul kaydınız Süper Admin onayına gönderildi. Onay tamamlandığında müdüre bildirim gönderilecek ve sistem kullanıma açılacaktır.":rejected?"Kurum kaydınız Süper Admin tarafından reddedildi. Ayrıntı için sistem bildirimini veya yöneticinizle iletişimi kontrol edin.":tenantRequired?"Hesabınız aktif bir tenant kurumuna bağlı değil. Kurum yöneticinizle iletişime geçin.":notAuthenticated?"OkulOS modüllerine erişmek için giriş yapın.":"Sistem/tenant erişim kontrolü tamamlanamadı. Lütfen tekrar deneyin.")}</p><div className="mt-5 flex justify-center gap-2">{!notAuthenticated&&!tenantRequired?<Link to="/notifications" className="rounded-md border px-4 py-2 text-sm font-medium">Bildirimleri Kontrol Et</Link>:null}<Link to="/" className="rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground">{notAuthenticated?"Giriş Yap":"Giriş Ekranı"}</Link></div></div></div>;
}
