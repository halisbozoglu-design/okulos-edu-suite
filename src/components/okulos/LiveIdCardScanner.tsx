...
export function LiveIdCardScanner({ onStableReading, onClose }: { onStableReading: (reading: IdentityReading) => void; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    async function inspectFrame() {
      if (!activeRef.current || initializing || workingRef.current || !videoRef.current || !canvasRef.current || !workerRef.current) return;
      ...
      if (!context) { workingRef.current = false; setWorking(false); return; }
      ...
      try {
        ...
        if (reading) {
          ...
          const stable = isStableIdentityReadings(readingsRef.current);
          setStatus(stable ? "Kimlik okuması sabitlendi." : "Kimlik okunuyor; aynı sonucu tekrar bekleniyor…");
          if (stable) onStableReading(reading);
        } else setStatus("Kartı çerçevede sabit ve aydınlık tutun.");
      } catch { setStatus("Kart okunamadı; ışık ve odağı kontrol edin."); }
      finally { workingRef.current = false; setWorking(false); }
    }
    async function start() {
      if (!window.isSecureContext) { setError("Canlı kamera yalnızca güvenli bağlantıda (HTTPS) kullanılabilir. Manuel doğrulama seçeneğini kullanın."); return; }
      if (!navigator.mediaDevices?.getUserMedia) { setError("Bu tarayıcı kamera erişimini desteklemiyor. Manuel doğrulama seçeneğini kullanın."); return; }
      try {
  ...
  return <div className="fixed inset-0 z-50 flex min-h-screen flex-col bg-background">
    ...
    <div className="flex flex-1 flex-col items-center justify-center gap-5 p-4"><div className="relative w-full max-w-2xl overflow-hidden rounded-2xl border bg-muted shadow-sm"><video ref={videoRef} playsInline autoPlay muted className="aspect-video w-full object-cover"/><div className="pointer-events-none absolute inset-0 grid place-items-center p-5"><div className="aspect-[856/540] w-full max-w-[82%] rounded-xl border-2 border-primary/80 opacity-90"><span className="sr-only">ID-1 kart çerçevesi</span></div></div></div><canvas ref={canvasRef} className="hidden" aria-hidden="true"/><div className="flex max-w-lg items-center gap-2 text-center text-sm text-muted-foreground">{working ? <LoaderCircle className="size-4 animate-spin"/> : <Square className="size-3 fill-current"/>}{status}</div>{error ? <div role="alert" className="flex max-w-lg items-start gap-2 rounded-xl border border-destructive/30 bg-destructive/10 p-3 text-sm text-destructive"><ShieldAlert className="mt-0.5 size-4 shrink-0"/>{error}</div> : null}<p className="max-w-lg text-center text-xs text-muted-foreground">Fotoğraf/shutter yoktur. Yalnızca kart çerçevesindeki geçici görüntü yaklaşık saniyede bir OCR için işlenir.</p></div>
  </div>;
}
