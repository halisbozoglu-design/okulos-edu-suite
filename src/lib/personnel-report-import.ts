...
function splitPersonCell(v:string){const raw=clean(v);const tc=raw.match(/\((\d{11})\)/)?.[1]??"";const name=clean(raw.replace(/\(\d{11}\)/g,"").replace(/\bGörevde\b.*$/i,"").replace(/\bGorevde\b.*$/i,""));const status=raw.match(/\b(Görevde|Gorevde)\s+([^\s]+(?:\s*[-/]\s*[^\s]+)?)/i);return {fullName:name,tcIdentityNo:tc,personnelStatus:status?.[1]??"",gradeStep:clean(status?.[2]??"")};}
function splitTitleDuty(v:string){const raw=clean(v);const compact=raw.replace(/\s*\/\s*/g,"/");const known=["Müdür Yardımcısı","Müdür","Sözleşmeli Öğretmen(657 S.K. 4/B)","Öğretmen"];
  for(const duty of known){const needle=`/${duty}`;const pos=lower(compact).lastIndexOf(lower(needle));if(pos>=0)return {baseTitle:clean(compact.slice(0,pos)),dutyTitle:duty};}
