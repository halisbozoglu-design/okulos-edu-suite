...
import {localAssignmentTeacherIds,localScopesOverlap,type LocalAssignment,type LocalLockedRow,type LocalScore,type LocalStudentConflictWeight} from "@/lib/schedule-local-solver-time-core";
...
export class IncrementalScheduleScore {
 ...
 constructor(private assignments:LocalAssignment[],private rows:LocalLockedRow[],private relations:PlanningRelation[]=[],weights:LocalStudentConflictWeight[]=[],private studentConflictMode:"MEDIUM"|"HARD"="MEDIUM"){this.index=new ScheduleHotspotIndex(assignments,[],weights);for(const r of rows)this.add(r);for(const rel of relations)this.dirty.add(rel.id)}
 private groupKeys(r:LocalLockedRow){const a=this.index.assignments.get(r.assignment_id),s=sig(a),teachers=a?localAssignmentTeacherIds(a):[r.teacher_id];return[...teachers.map(id=>k("t",id,r.weekday,s)),k("c",r.class_id,a?.subgroup_id??r.subgroup_id??"*",r.weekday,s)]}
 ...
 score(baseHard:number,unplaced:number):LocalScore{const r=this.relationTotal(),studentHard=this.studentConflictMode==="HARD"?this.student:0,studentMedium=this.studentConflictMode==="MEDIUM"?this.student:0;return{hard:baseHard+unplaced+r.hard+studentHard,medium:r.medium+studentMedium,soft:this.gaps*8+this.late*2+r.soft}}
