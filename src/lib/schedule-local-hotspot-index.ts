import { localAssignmentTeacherIds,localAssignmentsShareTeacher,localScopesOverlap,localStudentScopesOverlap, type LocalAssignment, type LocalLockedRow, type LocalStudentConflictWeight } from "@/lib/schedule-local-solver-time-core";
...
export class ScheduleHotspotIndex {
  ...
  private teacherIds(r:LocalLockedRow){const a=this.assignments.get(r.assignment_id);return a?localAssignmentTeacherIds(a):[r.teacher_id]}
  add(r:LocalLockedRow){this.put(this.slotRows,k(r.weekday,r.period),r);for(const id of this.teacherIds(r))this.put(this.teacherDayRows,k(id,r.weekday),r);this.put(this.classDayRows,k(this.classScope(r),r.weekday),r);const c=this.courseKey(r);if(c)this.put(this.courseClassDayRows,c,r);this.put(this.activityRows,this.activityKey(r),r)}
  remove(r:LocalLockedRow){this.drop(this.slotRows,k(r.weekday,r.period),r);for(const id of this.teacherIds(r))this.drop(this.teacherDayRows,k(id,r.weekday),r);this.drop(this.classDayRows,k(this.classScope(r),r.weekday),r);const c=this.courseKey(r);if(c)this.drop(this.courseClassDayRows,c,r);this.drop(this.activityRows,this.activityKey(r),r)}
  ...
  occupied(a:LocalAssignment,day:number,period:number){return this.slot(a,day,period).some(r=>localAssignmentsShareTeacher(a,this.assignments.get(r.assignment_id))||localStudentScopesOverlap(a,this.assignments.get(r.assignment_id)))}
  teacherDayFor(a:LocalAssignment,teacherId:string,day:number){return [...(this.teacherDayRows.get(k(teacherId,day))??[])].filter(r=>this.overlaps(a,r))}
  teacherDay(a:LocalAssignment,day:number){return [...new Set(localAssignmentTeacherIds(a).flatMap(id=>this.teacherDayFor(a,id,day)))]}
  ...
  teacherPeriodsFor(a:LocalAssignment,teacherId:string,day:number){return new Set(this.teacherDayFor(a,teacherId,day).map(r=>r.period))}
