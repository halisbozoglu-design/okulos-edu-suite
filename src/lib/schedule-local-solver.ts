...
export async function runLocalScheduleSolve(opts: { candidateCount: number; mode: LocalSolveMode; enableLns?: boolean; lnsIterations?: number; onProgress?: (p: LocalSolveProgress) => void }) {
  if (opts.mode === "DB") return { scenarioIds: [], capability: await detectLocalScheduleCompute(), generated: 0, complete: 0, useGpu: false, lns: null as LnsStats | null };
  const cap = await detectLocalScheduleCompute(), p = await problem(), useGpu = (opts.mode === "GPU" || opts.mode === "HYBRID" || opts.mode === "AUTO") && cap.webGpu, oversample = useGpu ? Math.max(opts.candidateCount * 4, 8) : Math.max(opts.candidateCount, strategies.length), workers = opts.mode === "GPU" ? 1 : Math.max(1, Math.min(cap.recommendedCpuWorkers, oversample)); let next = 0, done = 0; const out: Candidate[] = [];
  await Promise.all(Array.from({ length: workers }, async (_, wi) => { while (true) { const i = next++; if (i >= oversample) break; opts.onProgress?.({ worker: wi + 1, kind: "CPU", status: "running", completed: done, total: oversample }); const st = performance.now(); try { const candidate = await runWorker({ ...p, seed: (Date.now() + i * 7919) >>> 0, strategy: strategies[i % strategies.length], enableLns: opts.enableLns ?? true, lnsIterations: opts.lnsIterations }); out.push(candidate); done++; opts.onProgress?.({ worker: wi + 1, kind: "CPU", status: "done", completed: done, total: oversample, durationMs: Math.round(performance.now() - st) }); } catch { done++; opts.onProgress?.({ worker: wi + 1, kind: "CPU", status: "error", completed: done, total: oversample }); } } }));
  const complete = out.filter((x) => x.complete && x.failed === 0); let chosen = complete.slice().sort((a, b) => a.score.medium - b.score.medium || a.score.soft - b.score.soft).slice(0, opts.candidateCount);
  if (useGpu && complete.length) { opts.onProgress?.({ worker: 1, kind: "GPU", status: "running", completed: 0, total: complete.length }); const st = performance.now(); try { const scores = await gpuScoreScheduleCandidates(complete.map(metrics)); chosen = complete.map((c, i) => ({ c, s: scores[i] ?? Number.POSITIVE_INFINITY })).sort((a, b) => a.s - b.s).slice(0, opts.candidateCount).map((x) => x.c); opts.onProgress?.({ worker: 1, kind: "GPU", status: "done", completed: complete.length, total: complete.length, durationMs: Math.round(performance.now() - st) }); } catch { chosen = complete.slice().sort((a, b) => a.score.medium - b.score.medium || a.score.soft - b.score.soft).slice(0, opts.candidateCount); opts.onProgress?.({ worker: 1, kind: "GPU", status: "error", completed: 0, total: complete.length }); } }
  const scenarioIds: string[] = []; for (let i = 0; i < chosen.length; i++) { const candidate = chosen[i]; if (!candidate) continue; const rows = candidate.rows.map((r) => ({ assignment_id: r.assignment_id, weekday: r.weekday, period: r.period, classroom_id: r.classroom_id ?? null, subgroup_id: r.subgroup_id ?? null, locked: r.locked })); const q = await supabase.rpc("import_local_schedule_candidate_v1", { p_rows: rows, p_title: `Yerel ${candidate.strategy ?? (useGpu ? "GPU/CPU" : "CPU")} Adayı ${i + 1}` }); if (!q.error && q.data) scenarioIds.push(String(q.data)); }
  const bestLns = chosen.find((x) => x.lns?.enabled)?.lns ?? null; return { scenarioIds, capability: cap, generated: out.length, complete: complete.length, useGpu, lns: bestLns };
}
