...
import type { LnsStats, LocalCandidate } from "@/lib/schedule-local-solver-time-core";
import type { JointLocalProblem } from "@/lib/schedule-local-solver-incremental-core";
...
export async function loadLocalScheduleProblem() {
  const [a, l, u, t, c, p, rel, sw, tm,rooms,pools,bundles,members,bundleOptions] = await Promise.all([
    ...
    supabase.from("teacher_schedule").select("teacher_assignment_id,teacher_id,class_id,weekday,period,classroom_id,room_bundle_id,subgroup_id,schedule_session_id,locked").eq("active", true).eq("locked", true),
    ...
    supabase.from("classrooms").select("id,capacity,room_type,schedule_capabilities,building_id,room_pool_id").eq("active",true),
    supabase.from("schedule_room_pools").select("id,capacity,max_simultaneous_activities").eq("active",true),
    supabase.from("schedule_room_bundles" as any).select("id").eq("active",true),
    supabase.from("schedule_room_bundle_members" as any).select("bundle_id,classroom_id,member_role,ordinal").order("ordinal"),
    supabase.from("schedule_assignment_room_bundle_options" as any).select("teacher_assignment_id,bundle_id,preference_penalty"),
  ]);
  const e = a.error || l.error || u.error || t.error || c.error || p.error || rel.error || sw.error || tm.error||rooms.error||pools.error||bundles.error||members.error||bundleOptions.error; if (e) throw e;
  ...
  const poolById=new Map((pools.data??[]).map((x:any)=>[String(x.id),x])),memberByBundle=new Map<string,any[]>();
  for(const m of members.data??[]){const k=String((m as any).bundle_id);memberByBundle.set(k,[...(memberByBundle.get(k)??[]),m])}
  const optionMap:Record<string,string[]>={};for(const o of bundleOptions.data??[]){const k=String((o as any).teacher_assignment_id);optionMap[k]=[...(optionMap[k]??[]),String((o as any).bundle_id)]}
  const roomBundles:{room_bundle_id:string;primary_classroom_id:string;classroom_ids:string[]}[]=(bundles.data??[]).flatMap((b:any)=>{const ms=memberByBundle.get(String(b.id))??[],primary=ms.find((m:any)=>m.member_role==="PRIMARY");return primary&&ms.length>1?[{room_bundle_id:String(b.id),primary_classroom_id:String(primary.classroom_id),classroom_ids:ms.map((m:any)=>String(m.classroom_id))}]:[]});
  const roomIdsByBundle=new Map(roomBundles.map(b=>[b.room_bundle_id,b.classroom_ids]));
  return {
    ...
    rooms:(rooms.data??[]).map((x:any)=>{const pool:any=x.room_pool_id?poolById.get(String(x.room_pool_id)):null;return{classroom_id:String(x.id),capacity:Number(x.capacity),room_type:x.room_type??null,features:Array.isArray(x.schedule_capabilities)?x.schedule_capabilities.map(String):[],building_id:x.building_id?String(x.building_id):null,room_pool_id:x.room_pool_id?String(x.room_pool_id):null,pool_capacity:pool?Number(pool.capacity):null,max_simultaneous_activities:pool?Number(pool.max_simultaneous_activities):null}}),
    roomBundles,roomBundleOptions:optionMap,deferSingleRoomAssignment:true,
  };
}
...
function metrics(c: Candidate) { const byTeacher = new Map<string, number[]>(), byClass = new Map<string, number[]>(); let late = 0; for (const r of c.rows) { late += Math.max(0, r.period - 6); const tk = `${r.teacher_id}|${r.weekday}`, ck = `${r.class_id}|${r.weekday}`; byTeacher.set(tk, [...(byTeacher.get(tk) ?? []), r.period]); byClass.set(ck, [...(byClass.get(ck) ?? []), r.period]); } let gap = 0; for (const v of [...byTeacher.values(), ...byClass.values()]) { const a = [...new Set(v)].sort((x, y) => x - y); if (a.length) { const first = a[0] ?? 0, last = a.at(-1) ?? first; gap += Math.max(0, last - first + 1 - a.length); } } return { hard: c.score?.hard ?? 0, unplaced: c.failed, medium: c.score?.medium ?? 0, gap, late, repeat: 0, preference: 0 }; }
...
export async function runLocalScheduleSolve(opts: { candidateCount: number; mode: LocalSolveMode; enableLns?: boolean; lnsIterations?: number; seed?:number; onProgress?: (p: LocalSolveProgress) => void }) {
  ...
  const scenarioIds:string[]=[];for(let i=0;i<chosen.length;i++){const candidate=chosen[i];if(!candidate)continue;const rows=candidate.rows.map(r=>({assignment_id:r.assignment_id,weekday:r.weekday,period:r.period,classroom_id:r.classroom_id??null,room_bundle_id:r.room_bundle_id??null,subgroup_id:r.subgroup_id??null,locked:r.locked}));const q=await supabase.rpc("import_composite_room_candidate_v1" as any,{p_rows:rows,p_title:`Yerel Adaptif ${candidate.strategy??"CPU"} Adayı ${i+1}`});if(!q.error&&q.data)scenarioIds.push(String(q.data))}
