...
export function solveAdaptiveEliteSchedule(p:LocalProblem,opts?:{rounds?:number;eliteSize?:number;stagnationLimit?:number}):AdaptiveEliteCandidate{
 ...
 for(let round=0;round<rounds;round++){
  ...
  if(stagnation>=stagnationLimit){restarts++;stagnation=0;const restartStrategy=OPS[(round+restarts)%OPS.length]??"VND";const restart=solveIncrementalSchedule({...p,seed:deterministicSeed(p.seed,round,restarts*17),strategy:restartStrategy,enableLns:true,lnsIterations:32});addElite(elite,restart,eliteSize);if(lex(restart.score,best.score)<0)best=restart}
 }
