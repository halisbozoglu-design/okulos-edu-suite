...
Deno.serve(async req=>{
 ...
 try{
  ...
  const institutionCode=digits(body.institutionCode),schoolName=typeof body.schoolName==="string"?body.schoolName.trim():"",principalName=typeof body.principalName==="string"?body.principalName.trim():"",phone=normalizePhone(body.phone),email=emailNorm(body.email),tckn=digits(body.tckn),password=typeof body.password==="string"?body.password:"",redirectTo=allowedRedirect(body.redirectTo);
  ...
  const {error:institutionError}=await admin.from("institutions").insert({institution_code:institutionCode,school_name:schoolName,created_by:createdUserId});
