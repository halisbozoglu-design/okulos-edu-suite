-- Canonical clone behaviour for manual teacher assignments.
-- There is no reason, validity date or approval workflow. If the source
-- teacher/course assignment was manually confirmed, that flag follows the
-- same course into the target class. Timetable HARD rules are not changed.
...
begin
  ...
  if not exists(
    ...
    where id = p_source_class_id and public.tenant_row_allowed(institution_code)
  ) then raise exception 'SOURCE_CLASS_NOT_FOUND'; end if;
  if not exists(
    select 1 from public.school_classes
    where id = p_target_class_id and public.tenant_row_allowed(institution_code)
  ) then raise exception 'TARGET_CLASS_NOT_FOUND'; end if;
  ...
  set expected_weekly_hours = source.expected_weekly_hours, updated_at = now()
  ...
  if p_copy_teachers then
    insert into public.teacher_course_assignments(
      class_course_requirement_id, teacher_id, assigned_hours, assignment_group,
      note, created_by, is_manual_override, updated_at
    )
