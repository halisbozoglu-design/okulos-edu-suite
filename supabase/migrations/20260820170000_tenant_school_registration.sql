-- Tenant / institution foundation for OkulOS.
-- School principal T.C. is NEVER stored in clear text in this registration flow.
-- Only a masked value + non-reversible HMAC digest created by the Edge Function are stored.
...
-- School principals created through school registration do not need a clear-text T.C. in profiles.
...
$$;
revoke all on function public.is_institution_principal() from public;
grant execute on function public.is_institution_principal() to authenticated;
...
$$;
revoke all on function public.can_access_institution(text) from public;
grant execute on function public.can_access_institution(text) to authenticated;

-- Bridge the earlier personnel tenant logic to the canonical tenant membership table.
...
-- Tenant-scoped read policies. Recovery identity is intentionally NOT client-readable.
...
-- Compatibility with personnel privacy table from the previous migration.
