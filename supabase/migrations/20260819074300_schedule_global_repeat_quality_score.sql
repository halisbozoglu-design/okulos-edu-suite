-- The global max_same_course_per_day setting is a SOFT quality target.
-- Course-specific course_schedule_rules.max_per_day remains the HARD placement constraint.
-- Make repeated_course_penalty effective in scenario ranking so the UI setting is no longer inert.
...
begin
  ...
  -- Class timetable gaps (parallel subgroups count as one occupied class slot).
  select coalesce(sum(gaps),0)::integer into v from (
  ...
  -- Global same-course/day SOFT quality target.
  -- Count distinct class periods so parallel subgroup teachers do not multiply the class lesson count.
