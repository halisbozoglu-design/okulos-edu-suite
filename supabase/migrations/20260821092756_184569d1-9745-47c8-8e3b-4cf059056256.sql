...
grant select, insert, update, delete on public.personnel_registry to authenticated;
grant all on public.personnel_registry to service_role;
grant select, insert, update, delete on public.responsibility_catalog to authenticated;
grant all on public.responsibility_catalog to service_role;
grant select, insert, update, delete on public.personnel_responsibilities to authenticated;
grant all on public.personnel_responsibilities to service_role;
grant select, insert, update, delete on public.calendar_event_tasks to authenticated;
grant all on public.calendar_event_tasks to service_role;
grant select, insert, update, delete on public.calendar_task_files to authenticated;
grant all on public.calendar_task_files to service_role;
...
revoke all on function public.import_class_summaries(text,jsonb) from public, anon;
revoke all on function public.import_personnel_registry(text,jsonb) from public, anon;
grant execute on function public.import_class_summaries(text,jsonb) to authenticated, service_role;
...
create policy calendar_task_files_manage on public.calendar_task_files for all to authenticated using (exists(select 1 from public.calendar_event_tasks t where t.id=task_id and (t.assigned_user_id=auth.uid() or public.has_permission(auth.uid(),'settings.manage')))) with check (exists(select 1 from public.calendar_event_tasks t where t.id=task_id and (t.assigned_user_id=auth.uid() or public.has_permission(auth.uid(),'settings.manage'))));