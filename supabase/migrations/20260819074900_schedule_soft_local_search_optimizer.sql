-- Hard-safe local search: after repair, improve flexible single lessons using the real quality score.
-- Locked, synchronized, subgroup and explicit block rows are never mutated here.

alter table public.schedule_optimization_settings
  add column if not exists soft_local_search boolean not null default true,
  add column if not exists soft_local_search_moves smallint not null default 20 check(soft_local_search_moves between 0 and 100);
...
declare
  ...
  v_before_score integer;v_after_score integer;v_before_hard integer;v_after_hard integer;
...
begin
  ...
  loop
    ...
    select coalesce(sum(affected_count),0)::integer into v_before_hard from public.get_schedule_scenario_hard_issues_v2(p_scenario_id);
    ...
    foreach d in array v_profile.teaching_days loop
      ...
      for p in 1..v_profile.periods_per_day loop
        ...
        begin
          ...
          select coalesce(sum(affected_count),0)::integer into v_after_hard from public.get_schedule_scenario_hard_issues_v2(p_scenario_id);
          if v_after_hard<=v_before_hard then
  ...
  if v_moves>0 then
    ...
    values(p_scenario_id,v_action_no,'SOFT_LOCAL_SEARCH',format('Hard kuralları bozmadan %s esnek ders daha iyi slota taşındı.',v_moves),v_end_score-v_start_score,
      (select coalesce(sum(affected_count),0) from public.get_schedule_scenario_hard_issues_v2(p_scenario_id)),
      (select coalesce(sum(affected_count),0) from public.get_schedule_scenario_hard_issues_v2(p_scenario_id)),
      jsonb_build_object('score',v_start_score),jsonb_build_object('score',v_end_score));
  end if;
  return v_moves;
end;$$;

-- Insert local search into the existing rescore/explanation path.
alter function public.rescore_schedule_scenario_permission_core_v2(uuid)
rename to rescore_schedule_scenario_pre_local_v4;
create or replace function public.rescore_schedule_scenario_permission_core_v2(p_scenario_id uuid)
returns integer language plpgsql security definer set search_path=public as $$
declare v integer;s public.schedule_optimization_settings%rowtype;
begin
  select * into s from public.schedule_optimization_settings where id=true;
  if coalesce(s.soft_local_search,true) and coalesce(s.soft_local_search_moves,0)>0 then
    perform public.improve_schedule_scenario_soft_v1(p_scenario_id,s.soft_local_search_moves);
  end if;
  v:=public.rescore_schedule_scenario_pre_local_v4(p_scenario_id);
  return v;
end;$$;
