-- Higher operational permissions imply the minimum read access required to perform the task.
-- This is one-way: read permissions never imply edit/approve/publish permissions.
...
-- Payroll matrix is required by every payroll operation, not only explicit payroll.view.
...
-- Read policies needed by delegated operators.
...
-- Operational profile lists (teacher names, etc.) are needed by delegated task screens.
