-- Advanced optimization engine: profile weights, hard/soft/off rules, duty integration,
-- pedagogic load, workshop block quality, explainable scoring and repair-compatible validation.
...
-- Workshop policies feed the existing block-pattern solver instead of creating a second solver.
...
-- Reusable score breakdown for explainability.
...
-- Preserve the proven base scorer and extend it additively.
...
-- Configurable hard rules are appended to the existing hard-validation authority.
...
-- Refresh explanation after the proven rescore path completes.
