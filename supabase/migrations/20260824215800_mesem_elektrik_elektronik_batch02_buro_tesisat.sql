...
-- Güvenli alt-batch 02: Büro Makineleri Teknik Servisi + Elektrik Tesisatları ve Pano Montörlüğü
...
select name,category,true from (values
 ('Baskı Makineleri','uygulama'),('Fakslar','uygulama'),('Dijital Elektronik','uygulama'),('Yazar Kasa ve Para Sayma Makinesi','uygulama'),('Yazıcılar','uygulama'),('Mikrokontrol Devreleri','uygulama'),
 ('Pano Tasarım ve Montajı','uygulama'),('Zayıf Akım Tesisleri','uygulama'),('Bilgisayarlı Elektrik Tesisat Projeleri Çizimi','uygulama'),('Yapı Elektrik ve Kuvvet Tesisleri','uygulama')
) v(name,category)
on conflict(name) do update set active=true;

with branches(branch_name,source_page) as (values ('Büro Makineleri Teknik Servisi',11::smallint),('Elektrik Tesisatları ve Pano Montörlüğü',12::smallint)),
grades(grade_level,common_u,common_d,voc,elective,total_u,total_d) as (values
 (9::smallint,6::smallint,8::smallint,2::smallint,2::smallint,42::smallint,44::smallint),(10,6,8,4,0,42,44),(11,3,7,7,0,42,46),(12,2,8,8,0,42,48)),
...
select '2026-2027','MESEM','MESEM','Elektrik-Elektronik Teknolojisi',b.branch_name,g.grade_level,(case when v.schedule_variant='USTALIK' then g.common_u else g.common_d end)+g.voc+32,0,0,0,g.elective,case when v.schedule_variant='USTALIK' then g.total_u else g.total_d end,case when v.schedule_variant='USTALIK' then g.total_u else g.total_d end,case when v.schedule_variant='USTALIK' then g.total_u else g.total_d end,'[]'::jsonb,'https://meslek.meb.gov.tr/upload/cop10_mem/2021_elektrik_mem_cop.pdf','2021-33','Parantezli temel ders saatleri diploma programı için ilave fark dersleridir; ustalık ve diploma profilleri ayrı tutulur.',true,v.schedule_variant,case when v.schedule_variant='USTALIK' then g.common_u else g.common_d end,g.voc,0,g.elective,0,32,'CURRENTLY_VALID',b.source_page,'{"enterpriseDays":4,"enterpriseHoursPerDay":8,"parentheticalHoursMeaning":"DIPLOMA_ADDITIONAL_DIFFERENCE_COURSES"}'::jsonb,0
...
with b(branch_name,source_page) as (values ('Büro Makineleri Teknik Servisi',11::smallint),('Elektrik Tesisatları ve Pano Montörlüğü',12::smallint)),
...
branch_courses(branch_name,grade_level,course_name,hours) as (values
 ('Büro Makineleri Teknik Servisi',11,'Baskı Makineleri',2),('Büro Makineleri Teknik Servisi',11,'Fakslar',2),('Büro Makineleri Teknik Servisi',11,'Dijital Elektronik',2),('Büro Makineleri Teknik Servisi',11,'Endüstriyel Kontrol ve Arıza Analizi',1),
 ('Büro Makineleri Teknik Servisi',12,'Yazar Kasa ve Para Sayma Makinesi',1),('Büro Makineleri Teknik Servisi',12,'Yazıcılar',2),('Büro Makineleri Teknik Servisi',12,'Mikrokontrol Devreleri',2),('Büro Makineleri Teknik Servisi',12,'Bilgisayar Destekli Uygulamalar',2),
 ('Elektrik Tesisatları ve Pano Montörlüğü',11,'Pano Tasarım ve Montajı',4),('Elektrik Tesisatları ve Pano Montörlüğü',11,'Zayıf Akım Tesisleri',2),('Elektrik Tesisatları ve Pano Montörlüğü',11,'Endüstriyel Kontrol ve Arıza Analizi',1),
 ('Elektrik Tesisatları ve Pano Montörlüğü',12,'Bilgisayarlı Elektrik Tesisat Projeleri Çizimi',3),('Elektrik Tesisatları ve Pano Montörlüğü',12,'Yapı Elektrik ve Kuvvet Tesisleri',2),('Elektrik Tesisatları ve Pano Montörlüğü',12,'Bilgisayar Destekli Uygulamalar',2)),
rows as (
