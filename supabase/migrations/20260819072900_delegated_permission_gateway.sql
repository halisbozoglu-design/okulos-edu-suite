...
-- Delegated users are NEVER globally promoted to manager. A wrapper verifies one permission,
-- sets a transaction-local permission context, then calls the proven core implementation.
...
-- Deliberately NOT granted to authenticated; only SECURITY DEFINER wrappers call it.
...
-- Read functions intentionally remain readable according to their existing policies/API.
...
-- Explicit grants for public wrapper entry points.
