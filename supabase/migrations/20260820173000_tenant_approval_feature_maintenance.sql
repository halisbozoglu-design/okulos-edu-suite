-- Tenant approval, super-admin messaging and global module maintenance.
-- The first school itself is safe to seed by public institution code/name.
-- Principal phone/T.C./password are intentionally NOT embedded in source control.
...
-- First tenant seed. Sensitive principal credentials stay out of Git history.
...
-- If the existing Super Admin account is already present, bind it as the first school's principal-owner too.
-- This does not copy or store T.C./phone/password.
do $$
declare v_uid uuid;
begin
...
$$;
revoke all on function public.super_admin_list_tenants() from public;
grant execute on function public.super_admin_list_tenants() to authenticated;
...
revoke all on function public.notify_tenant_principals(text,text,text,text) from public;
...
revoke all on function public.super_admin_review_tenant(text,text,text) from public;
grant execute on function public.super_admin_review_tenant(text,text,text) to authenticated;
...
revoke all on function public.super_admin_send_tenant_message(text,text,text,text) from public;
grant execute on function public.super_admin_send_tenant_message(text,text,text,text) to authenticated;
...
revoke all on function public.super_admin_set_system_maintenance(boolean,text) from public;
grant execute on function public.super_admin_set_system_maintenance(boolean,text) to authenticated;
...
$$;
revoke all on function public.get_system_feature_matrix() from public;
grant execute on function public.get_system_feature_matrix() to authenticated;
