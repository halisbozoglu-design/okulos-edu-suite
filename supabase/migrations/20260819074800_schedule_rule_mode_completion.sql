-- Complete HARD/SOFT/OFF semantics for advanced rules and make course time preferences effective.

update public.schedule_rule_modes set config=jsonb_set(config,'{max_daily_load_spread}','10'::jsonb,true)
where rule_code='pedagogic_daily_balance' and not(config ? 'max_daily_load_spread');
update public.schedule_rule_modes set config=jsonb_set(config,'{difficulty_threshold}','4'::jsonb,true)
where rule_code='heavy_course_consecutive' and not(config ? 'difficulty_threshold');

insert into public.schedule_rule_modes(rule_code,label,category,mode,weight,config)
values('course_time_preference','Dersin sabah/gün tercihi','pedagogy','soft',12,'{}'::jsonb)
on conflict(rule_code) do nothing;

alter function public.get_schedule_scenario_quality_breakdown_v1(uuid)
rename to get_schedule_scenario_quality_breakdown_pre_time_v1;
...
language sql stable security definer set search_path=public as $$
  select * from public.get_schedule_scenario_quality_breakdown_pre_time_v1(p_scenario_id)
  union all
  select 'course_time_preference',
    case when public.schedule_rule_mode_v1('course_time_preference')='off' then 0 else coalesce(sum(
      case
        when pp.prefer_early and r.period>4 then public.schedule_rule_weight_v1('course_time_preference','late_period',12)
        when pp.avoid_early and r.period<=2 then public.schedule_rule_weight_v1('course_time_preference','late_period',12)
        when cardinality(pp.prefer_weekdays)>0 and not(r.weekday=any(pp.prefer_weekdays)) then public.schedule_rule_weight_v1('course_time_preference','late_period',12)
        else 0 end
    ),0)::integer end,
    'Dersin tanımlı sabah/kaçınma/tercih edilen gün kuralına uyumu.'
  from public.schedule_scenario_rows r join public.course_pedagogy_profiles pp on pp.course_id=r.course_id
...
heavy_bad as (
  select count(*)::integer n from public.schedule_scenario_rows a join public.schedule_scenario_rows b
    on b.scenario_id=a.scenario_id and b.class_id=a.class_id and b.weekday=a.weekday and b.period=a.period+1
  ...
  where a.scenario_id=p_scenario_id and coalesce(pa.difficulty,3)>=coalesce((select (config->>'difficulty_threshold')::integer from public.schedule_rule_modes where rule_code='heavy_course_consecutive'),4)
...
time_pref_bad as (
  select count(*)::integer n from public.schedule_scenario_rows r join public.course_pedagogy_profiles pp on pp.course_id=r.course_id
  where r.scenario_id=p_scenario_id and ((pp.prefer_early and r.period>4) or (pp.avoid_early and r.period<=2) or (cardinality(pp.prefer_weekdays)>0 and not(r.weekday=any(pp.prefer_weekdays))))
)
