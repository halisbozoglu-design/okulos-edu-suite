...
def solve(data,limit_s=30.0,seed=1):
    ...
    hard_unsupported=any(x['mode']=='HARD' for x in unsupported)
    ...
    for d in days:
        for p in periods:
            teachers={str(a.get('teacher_id')) for a in assignments}; classes={str(a.get('class_id')) for a in assignments}
            for tid in teachers:
                vs=[x[(str(a['assignment_id']),d,p)] for a in assignments if str(a.get('teacher_id'))==tid and (str(a['assignment_id']),d,p) in x]
                if len(vs)>1:model.Add(sum(vs)<=1)
            for cid in classes:
                vs=[x[(str(a['assignment_id']),d,p)] for a in assignments if str(a.get('class_id'))==cid and (str(a['assignment_id']),d,p) in x]
                if len(vs)>1:model.Add(sum(vs)<=1)
    ...
    for c in data.get('student_conflict_weights') or []:
        la,ra=str(c.get('left_assignment_id')),str(c.get('right_assignment_id')); w=float(c.get('severity_weight') or c.get('student_weight') or 1)
        for d in days:
            for p in periods:
                k1,k2=(la,d,p),(ra,d,p)
        ...
        for a in chosen:
            ...
            for d in days:
                for p in periods:
                    ...
                    hit=(d,p) in ss
                    violation=(typ=='FORBIDDEN_SLOT' and hit) or (typ=='PREFERRED_SLOT' and not hit)
                    if not violation:continue
                    if mode=='HARD':model.Add(x[key]==0)
                    elif mode in ('MEDIUM','SOFT'):add_pen(x[key],weight,mode)
    med_max=sum(w for _,w in medium);soft_max=sum(w for _,w in soft);scale=soft_max+1
