...
export async function runWorldBenchmark(opts:{seedCount?:number;ci?:boolean}={}){
 ...
 for(const profile of manifest.profiles){const times:number[]=[],timedSeeds:{seed:number;ms:number}[]=[],first:number[]=[],best:number[]=[],hard:number[]=[],unplaced:number[]=[],medium:number[]=[],soft:number[]=[];let feasible=0;const heap0=process.memoryUsage().heapUsed;let inputHash="",replay=true;
...
export function assertWorldBenchmarkGate(report:WorldBenchmarkReport,minSeeds=manifest.seed_count){
 ...
 for(const r of report.results){if(r.runs<minSeeds||r.feasible_rate!==1||r.hard_max!==0||r.unplaced_max!==0||!r.deterministic_replay||!r.budget_pass)throw new Error(`WORLD_BENCH_GATE_FAILED:${r.profile_id}`)}
}

if(import.meta.main){const args=process.argv.slice(2),ci=args.includes("--ci"),oi=args.indexOf("--out"),out=oi>=0?args[oi+1]:null,seedCount=readPositiveIntegerFlag(args,"--seeds",manifest.seed_count),minSeeds=readPositiveIntegerFlag(args,"--min-seeds",manifest.seed_count);const report=await runWorldBenchmark({seedCount,ci});console.log("SCHEDULE_WORLD_BENCH",JSON.stringify(report));if(out)await Bun.write(out,JSON.stringify(report,null,2));assertWorldBenchmarkGate(report,minSeeds)}
