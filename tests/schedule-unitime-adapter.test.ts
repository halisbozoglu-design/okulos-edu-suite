...
describe("UniTime external benchmark adapter",()=>{
 ...
 test("uses seeded fixed-iteration search so replay is not wall-clock jitter dependent",()=>{expect(src).toContain("ToolBox.setSeed(seed)");expect(src).toContain("DETERMINISTIC_MAX_ITERS = 100_000");expect(src).toContain('Termination.MaxIters",Integer.toString(DETERMINISTIC_MAX_ITERS)');expect(src).toContain('Termination.TimeOut","-1');});
