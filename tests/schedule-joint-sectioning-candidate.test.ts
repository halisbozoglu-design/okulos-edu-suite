...
describe("joint timetable-sectioning persistence contract",()=>{
 ...
 test("browser runner sends one combined candidate through composite-aware server audit",()=>{expect(runner).toContain("solveJointScheduleSectioning");expect(runner).toContain("import_joint_composite_schedule_candidate_v1");expect(runner).toContain("student_free_time_requests");expect(ui).toContain("Joint Aday Üret")});
});
