...
describe("world benchmark package",()=>{
 ...
 test("competitors are never fabricated",async()=>{for(const id of["timefold","unitime","fet","asc"]){const s=manifest.solvers.find((x:any)=>x.id===id);expect(["NOT_RUN","RUN_COMMON_HARD"]).toContain(s?.status);if(s?.status==="RUN_COMMON_HARD"){expect(s?.adapter).toBeTruthy();expect(s?.evidence).toBeTruthy();expect(s?.comparable_objective).toBe(false);expect(await Bun.file(s.evidence).exists()).toBe(true)}else expect(s?.adapter_contract).toBeTruthy()}expect(readme).toContain("never replaced by estimates")});
