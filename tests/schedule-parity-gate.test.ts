...
describe("final parity/superiority truth gate",()=>{
 ...
 test("common-hard evidence cannot unlock superiority",async()=>{const r=await evaluateParityGate();expect(r.superiority_claim_allowed).toBe(false);expect(r.competitor_not_comparable.map(x=>x.engine)).toEqual(expect.arrayContaining(["Timefold","UniTime","FET","aSc"]))});
});
