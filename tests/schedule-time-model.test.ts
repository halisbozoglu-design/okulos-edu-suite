...
import {localScopesOverlap,type LocalProblem} from "../src/lib/schedule-local-solver-time-core";
import {solveIncrementalSchedule as solveLocalSchedule} from "../src/lib/schedule-local-solver-incremental-core";
...
const incremental=await Bun.file("src/lib/schedule-local-solver-incremental-core.ts").text();
...
describe("schedule canonical time model",()=>{
 ...
 test("browser worker consumes incremental core that reuses canonical time scope",()=>{expect(worker).toContain("schedule-local-solver-incremental-core");expect(incremental).toContain("localScopesOverlap");expect(incremental).toContain("IncrementalScheduleScore");expect(localLoader).toContain("get_schedule_assignment_time_model_v1");expect(localLoader).toContain("allowed_periods");expect(localLoader).toContain("week_pattern")});
